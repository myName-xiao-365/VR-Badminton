...
namespace VRBadminton.Gameplay
{
    public sealed class ShuttleFeedController : MonoBehaviour
    {
        ...
        [SerializeField] private float racketContactHalfWidth = 0.48f;
        [SerializeField] private float racketContactHalfHeight = 0.56f;
        [SerializeField] private float racketContactDepth = 0.52f;
        ...
        private void OnGUI()
        {
            ...
            if (incomingOpponentSmash)
            {
                ...
                GUI.Label(
                    ...
                    smashReceiveReady ? "READY - SWING UP" : "PRESS SPACE TO DEFEND",
                    uiLabelStyle);
            }
        ...
        private bool TryHitShuttle(out ShotType shot)
        {
            ...
            bool racketContact = IsShuttleTouchingRacketFace();
            if (!playerInPosition || !racketContact)
            {
                return false;
            }
        ...
        private bool IsShuttleTouchingRacketFace()
        {
            Vector3 localShuttlePosition =
                racketFace.InverseTransformPoint(shuttle.position);
            return Mathf.Abs(localShuttlePosition.x) <= racketContactHalfWidth &&
                Mathf.Abs(localShuttlePosition.y) <= racketContactHalfHeight &&
                Mathf.Abs(localShuttlePosition.z) <= racketContactDepth;
        }
